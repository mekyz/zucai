...
<script>
	if(isMobileWeb()){
		window.location.href="transaction_detail_m";
	}
</script>
</head>
<body>
		...
		<div id="div_data" class="col-lg-12 col-md-12 col-sm-12 col-xs-12">
			<div class="panel panel-default bk-bg-white">
				<div class="panel-heading bk-bg-white">
					<h6>
						<i class="fa fa-indent red"></i>投注记录
					</h6>
					...
					<table class="table table-no-more table-bordered table-striped mb-none">
						<thead>
							<tr>
								...
								<c:choose>
									<c:when test="${userInfo.userType >= 50}">
										<th class="text-center">团队投注金额</th>
									</c:when>
								</c:choose>
						...
						<tbody>
							<tr v-for="(item,index) in betData.userBetInfoList">
								...
								<c:choose>
									<c:when test="${userInfo.userType >= 50}">
										<td data-title="团队投注金额" class="text-center">{{item.teamBetMoney}}</td>
									</c:when>
								</c:choose>
								<td data-title="状态" class="text-center">{{item.statusStr}}</td>
							</tr>
						</tbody>
					</table>
					<div class="row">
	...
	<script>
		...
		function getMatchInfo(userBetInfo){
			ajaxJson("ajaxGetMatchProfitInfo","profitId="+userBetInfo.profitId,function(msg){
				...
				matchProfitInfo.profitPercentStr=(userBetInfo.money*matchProfitInfo.profitPercent/1000000).toFixed(2);
				...
				ajaxJson("ajaxGetMatchInfo","phaseId="+matchProfitInfo.phaseId,function(msg){
					...
					var current=getCurrentTimeLong();
					if(matchInfo.matchDate>current||matchInfo.halfResultStatus!=LR_WEB.STATUS.ENABLED||matchInfo.finalResultStatus!=LR_WEB.STATUS.ENABLED){
						getTeamBetMoney(userBetInfo);
					}
				},function(errmsg){});
			},function(errmsg){});
		}
		function getTeamBetMoney(userBetInfo){
			ajaxJson("ajaxGetUserTeamBetMoney","phaseId="+userBetInfo.phaseId,function(msg){
				var money=toJson(msg);
				userBetInfo.teamBetMoney=convertRmb(money);
			},function(errmsg){});
		}
		...
		$(function(){
			app1=new Vue({el:'#app1',data:{betData:{userBetInfoList:[],LIST_LEN:LR_WEB.LIST_LEN,totalRecords:0,totalPage:0,currentPage:1,divData:'div_data',divNoData:'div_no_data'},isLoading:false}});
			initData(app1);
			getMoreData();
		});
	</script>
