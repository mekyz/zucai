...
<script>
if(isMobileWeb()){
	window.location.href="wdrecord_m";
}
</script>
</head>
<body>
					...
					<table class="table table-no-more table-bordered table-striped mb-none">
						...
						<tbody>
							<tr v-for="(item,index) in withdrawData.withdrawInfoList">
								...
								<td data-title="提现留言" class="text-center">{{item.remark}}</td>
								<td data-title="审批留言" class="text-center">{{item.payRemark}}</td>
								<td data-title="申请状态" class="text-center">{{item.statusStr}}</td>
								<td data-title="审批时间" class="text-center">{{item.verifyDateLongStr}}</td>
							</tr>
						</tbody>
					</table>
					<div class="row">
