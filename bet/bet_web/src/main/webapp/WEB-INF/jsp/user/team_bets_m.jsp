...
<script>
	if(!isMobileWeb()){
		window.location.href="team_bets";
	}
</script>
</head>
<body>
					...
					<table class="table table-no-more table-bordered table-striped mb-none">
						...
						<tbody>
							<tr v-for="(item,index) in userData.userInfoList">
								...
								<td data-title="下注金额" class="text-center" style="height: 38px;">{{item.totalMoney}}</td>
							</tr>
						</tbody>
					</table>
					<div class="row">
	...
	<script>
		...
		function getBetMoney(userInfo){
			ajaxJson("ajaxGetUserBetTotalMoney","userId="+userInfo.userId+"&startDateLong="+startDateLong+"&endDateLong="+endDateLong,function(res){
				showLog(userInfo.userId+' res:'+res);
				var totalMoney=res;
				...
				showLog(userInfo.userId+'errmsg:'+errmsg);
				app1.userData.userInfoList.push(userInfo);
			});
		}
		...
		$(function(){
			app1=new Vue({el:'#app1',
				data:{userData:{userInfoList:[],LIST_LEN:LR_WEB.LIST_LEN,totalRecords:0,totalPage:0,currentPage:1,divData:'div_data',divNoData:'div_no_data'},isLoading:false,totalMoney:0}});
			initData(app1);
			//getAll();
