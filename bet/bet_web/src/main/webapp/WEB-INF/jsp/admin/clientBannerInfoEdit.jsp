...
<title>修改客户端Banner</title>
<meta name="pageId" content="clientBannerEdit" />
...
<script>
	...
	function formSubmit(){
		submitForm("clientBannerInfo","ajaxUpdateClientBannerInfo",true,"请稍后...",function(data){
	...
	function getPartnerList(){
		ajaxDataJson("ajaxGetUserInfoList","userType="+LR_WEB.USER_TYPE.PARTNER,
			function(data){
				var partnerList=data.data;
				var content="<option value =''>请选择</option>";
				var currentPartnerId='${clientBannerInfo.partnerId}';
				if(partnerList.length>0){
					for( var index in partnerList){
						var partnerInfo=partnerList[index];
						content+="<option value ='"+partnerInfo.userId+"' "+(partnerInfo.userId==currentPartnerId?"selected='selected'":"")+">"+partnerInfo.nickname+"("+partnerInfo.userId+")"
							+"</option>";
					}
				}
	...
	function getAgentList(){
		...
		ajaxDataJson("ajaxGetUserInfoList","partnerId="+partnerId+"&userType="+LR_WEB.USER_TYPE.AGENT,function(data){
			...
			var content="<option value =''>请选择</option>";
			var currentAgentId='${clientBannerInfo.agentId}';
			if(agentList.length>0){
				for( var index in agentList){
					var agentInfo=agentList[index];
					content+="<option value ='"+agentInfo.userId+"' "+(agentInfo.userId==currentAgentId?"selected='selected'":"")+">"+agentInfo.nickname+"("+agentInfo.userId+")"+"</option>";
				}
			}
						...
						<div class="form-group">
							...
							<div class="col-md-6 col-sm-6 col-xs-12">
								...
								<div class="col-xs-12" style="padding-left: 0; padding-right: 0;">
									<img id="uploadImg" width="50%" src="<%=basePath%>${clientBannerInfo.picUrl}" />
								</div>
							</div>
						</div>
						<div class="form-group">
