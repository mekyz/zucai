...
/*
 * Simplified implementation of DOM2 EventTarget. http://www.w3.org/TR/DOM-Level-2-Events/events.html#Events-EventTarget
 */
...
if (
    ...
    (function () {
        ...
        StringPrototype.split = function (separator, limit) {
            ...
            /*
			 * Values for `limit`, per the spec: If undefined: 4294967295 // Math.pow(2, 32) - 1 If 0, Infinity, or NaN: 0 If positive number: limit = Math.floor(limit); if (limit > 4294967295) limit -=
			 * 4294967296; If negative number: 4294967296 - Math.floor(Math.abs(limit)) If other: Type-convert, then use the above rules
			 */
            limit = limit === void 0 ?
                -1 >>> 0 : // Math.pow(2, 32) - 1
                ToUint32(limit);
...
/**
 * Currently only WebKit-based Web Inspectors, Firefox >= v31, and the Firebug extension (any Firefox version) are known to support "%c" CSS customizations. TODO: add a `localStorage` variable to
...
/**
 * Parse or format the given `val`. Options: - `long` verbose formatting [false]
...
/**
 * These are the parse rules for the URL parser, it informs the parser about: 0. The char it Needs to parse, if it's a string it should be done using indexOf, RegExp using exec and NaN means set as
 * current value. 1. The property we should set when parsing this value. 2. Indication if it's backwards or forward parsing, when set as number it's the value of extra chars that should be split off.
 * 3. Inherit from location if non existing in the parser. 4. `toLowerCase` the resulting value.
 */
var rules = [
 ...
 * @param {Boolean|Function} fn When setting the query, it will be the function used to parse the query. When setting the protocol, double slash will be removed from the final url if it is true.
 * @returns {URL}
 * @api public
 */
...
/**
 * These properties should not be copied or inherited from. This is only needed for all non blob URL's as a blob URL does not include a hash, only the origin.
 * @type {Object}
 * @private
 */
...
/**
 * The location object differs when your code is loaded through a normal page, Worker or through a worker using a blob. And with the blobble begins the trouble as the location object will contain the
 * URL of the blob, not the location of the page where our code is loaded in. The actual origin is encoded in the `pathname` so we can thankfully generate a good "default" location from it so we can
 * generate proper relative URL's again.
